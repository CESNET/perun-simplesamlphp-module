...
?>
    ...
    <form method="post" action="<?php echo SimpleSAML_Module::getModuleURL('perun/force_aup_continue.php'); ?>" >
        ...
        foreach ($newAups as $key => $aup) {
			echo '<div>';
			echo '<p style="font-size: 16px; padding: 0; margin: 0;">' . $this->t('{perun:perun:organization}') . '<strong>' . $key . '</strong></p>';
			echo '<p>' . $this->t('{perun:perun:force-aup-tpl_aup_redirect}') . $aup->version . ' <a href="' . $aup->link . '">' . $this->t('{perun:perun:here}'). '</a></p>';
            echo '</div>';
		}
        ?>
